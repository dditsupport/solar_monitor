...
// the DS1307 is running ahead of true time; negative = behind. Growing
...
// Supply/battery voltage (V) captured with the same hourly sample and reported
// next to the drift and RSSI. 0 = not captured.
static float    s_drift_batt_v   = 0.0f;
  ...
  while (millis() - start < NTP_SYNC_TIMEOUT_MS) {
    ...
    if (now > 1700000000) {
      ...
      if (rtc_now > 0 &&
        ...
        // Latest battery reading from the sampling task (ADC1/GPIO35).
        if (state_lock()) { s_drift_batt_v = g_state.battery_v; state_unlock(); }
        s_drift_pending  = true;
        s_last_drift_us  = now_us;
        LOG_PRINTF("[wifi] RTC drift sample: %+ld sec, RSSI %d dBm, batt %.2f V\n",
                   s_drift_sec, s_drift_rssi, s_drift_batt_v);
      }
...
static bool post_batch(uint64_t snapshot_seq, uint64_t &out_acked_seq) {
  ...
  if (srv_time && !time_source::wall_clock_known()) {
    ...
    if (srv_epoch > 0 && time_source::set_wall_clock(srv_epoch)) {
      ...
      // unavailable due to lost-power; this clears the clock-halt bit).
